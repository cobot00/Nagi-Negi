...
RSpec.describe Naginegi::MySQL::TableConfig do
  describe '.generate_table_configs' do
    subject { Naginegi::MySQL::TableConfig.generate_table_configs('spec/support/table.yml') }
    let(:db01_hoge) { Naginegi::MySQL::TableConfig.new({ 'name' => 'hoge', 'daily_snapshot' => true }) }
    let(:db01_simple) { Naginegi::MySQL::TableConfig.new({ 'name' => 'simple' }) }
    let(:db02_fuga) { Naginegi::MySQL::TableConfig.new({ 'name' => 'fuga' }) }
    let(:db02_with_condition) { Naginegi::MySQL::TableConfig.new({ 'name' => 'with_condition', 'condition' => 'created_at < CURRENT_DATE()' }) }
